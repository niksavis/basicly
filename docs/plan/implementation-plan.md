...
| The subcommand dispatch guard, derived from the parser | `basicly-8ry8` | **Closed.** All eight sites (seven groups plus the top level) now route through one `_dispatch` helper that exits 2 naming the subcommand. The regression test derives its site list from the parser and is parametrised over every site, with a positive control that fails if the derivation stops recursing — the old test asserted `len(actions) == 1` against the root parser and so never reached a nested group, which is why the seventh site was missed. |
| The tracker corrupts its WAL under the engine's own fan-out | `basicly-vkh0.10` | **Closed.** Carried forward as R7 in the work-tracker requirements register. Writing the gate found **our own** instance of the same defect: `scrub_export` rewrote the shared export in place while every lane read it through `.beads/redirect`, and `export_records` skips an unparseable line rather than raising, so a torn read returned a partial issue set with no error. Now published through a pid-scoped temp and a rename; `DATABASE_ERROR` is transient and backed off; a store loss is charged to `TRACKER_GATE`, not the lane's rework budget. Gated by four reader processes against a live writer — reverting the atomic write turns it red at 1669 of 3000 records. |
| A failed dispatch that never started an agent halts the grant | `basicly-jr0l.64` | **Closed, but it does not close the incident.** Records carry an `unstarted` outcome and `session_spend` no longer counts one as unmeterable; a completed run whose usage could not be parsed still halts. The lane then established that the 2026-08-02 halt was **not** the failed lane at all — the `tokens: 182` record is `phase: decide`, the decider agent invoked on the escalation. See the row below. |
| Every delegated decision halts an L2+ grant | `basicly-gczc` | **Open, P0.** `decisions.py:655` calls `runner.run` without `capture_usage=True`, so every decider dispatch records `estimated: true` and zeroes the remaining budget. The comment directly above it claims the call is "bounded and metered like every other dispatch" — a claim committed beside the thing it is wrong about, the `ipx2` class. Not a one-liner: `rubrics.py:281` omits the flag _deliberately_, because it switches some adapters' stdout to JSON and the verdict is parsed as plain text. |
| The ceiling ratchets on a scope a lane declares about itself | `basicly-qorx` | **Open, P1.** `working_set_max` is derived from the largest completed estimate, and the estimate is a function of the lane's own `## Scope`, which the lane may rewrite while running. `fcls` did — three globs to nine, honestly — which moved a global constant and pushed its own estimate to 329606, undispatchable. The design punishes accurate scope reporting. `jr0l.54` is the home for the enforcement half. |
| A deferred child is dispatchable and holds its epic open | `basicly-toj6` | **Open, P1.** `supervise.py:319` defines open children as `status != "closed"`, so a `deferred` bead is sized into the band table, counted in the forecast, and never lets `all_children_closed` become true. A deferred bead can block a pass its ready siblings could afford. |
| The remaining multi-lane blocker | `jr0l.65` | An _answered_ needs-input still blocks every delegated ship until its bead closes. Now in band at 24000; it was refused at 112070 before `fcls`. |
