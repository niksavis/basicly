...
| Measure | Value |
| --- | --- |
| Engine modules (`src/basicly/*.py`) | 120 |
| Test files | 253 |
| `[[verify.checks]]` declared | 40 |
| …of which run in `--mode fast` | 35 |
| …of which run in `--mode full` | 39 |
...
observation, because its definitions are **wired**. That is not an
argument for its design; its loop is the declarative-phase pattern this repo already
rejected. It is the argument that the next unit of work on
...
| `research/2026-08-14-harness-board-design.md` | **Dated evidence** | The solution design the harness board was built from, imported from the branch it was written on once that board shipped. It is a record of what was decided on one date, not a plan: every unit it proposes has landed. `basicly-rn0o.6` cites it by name, which is why it is here rather than deleted. Delete when nothing cites it. |
