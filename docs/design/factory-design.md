...
| `[policy.sizing]` | `working_set_max`         | `56_000` tokens    | Above → engine refuses, agent must split. Derived from recorded lane outcomes, so it moves with the estimator: `64_000` chosen → `112_000` (`basicly-3w44`) → `56_000` once scope read-cost stopped sizing whole files (`basicly-fcls`) |
