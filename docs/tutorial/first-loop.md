...
basicly 0.18.15 in a real terminal, with that repo's generated bead-id prefix swapped for
...
```sh
uvx --from git+https://github.com/niksavis/basicly@v0.18.15 basicly <args>
```
...
```sh
uvx --from git+https://github.com/niksavis/basicly@v0.18.15 basicly install
```
...
```text
catalog lint: warning: skill listing is 2665 tokens against a 2000-token budget (1% of the 200000-token claude window a consumer gets), from 49 entries. The host drops descriptions least-invoked first, so the entries this overrun silences are the ones already hardest to reach. Retire a dead skill or shorten a description.
...
[merged] build -> verify: merged harness/myrepo-rq9r @ e999d104a41a (1 commit(s)) into main @ 2b41b2f; landing 0.9s (tracker-commit 0.7s, merge 0.2s)
...
```text
d61b7f7 chore(tracker): close the shipped track (myrepo-rq9r)
2b41b2f chore(worktree): merge a harness worktree back to its base
e999d10 docs: add a getting started note (myrepo-rq9r)
d6a3e55 chore(tracker): sync tracker state for the harness loop (myrepo-rq9r)
d4966ee chore(tracker): record the claim before provisioning (myrepo-rq9r)
289f98a chore: install basicly (myrepo-rq9r)
```
