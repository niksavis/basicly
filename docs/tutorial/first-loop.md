...
basicly 0.14.1 in a real terminal, with that repo's generated bead-id prefix swapped for
...
```sh
uvx --from git+https://github.com/niksavis/basicly@v0.14.1 basicly <args>
```
...
```text
engine: basicly 0.14.1
repo: consumer
catalog: installed by basicly 0.14.1 at ... (matches engine)
