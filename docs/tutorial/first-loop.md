...
basicly 0.13.0 in a real terminal, with that repo's generated bead-id prefix swapped for
...
```sh
uvx --from git+https://github.com/niksavis/basicly@v0.13.0 basicly <args>
```
...
```text
engine: basicly 0.13.0
repo: consumer
catalog: installed by basicly 0.13.0 at ... (matches engine)
