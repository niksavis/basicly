...
| `basicly permissions-build` / `permissions-check` | Project the agent-permissions deny-list into the co-owned settings file: ensure-present, consumer entries preserved, nothing pruned, with a semantic subset drift check. The check also reads `~/.claude/settings.json` and warns when `autoMode.environment` has no `"$defaults"` and its trusted repo or source control entry names one repository, because auto mode then reads every other repository as external. The warning prints the fix. The check never writes that file |
