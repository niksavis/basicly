...
| `basicly skills-user <names> [--home DIR] [--dry-run]` | Project the named catalog skills (patterns allowed, for example `tool-*`) into `~/.claude/skills` so every repo sees them. It writes the generated marker, prunes only marked skills, never overwrites an unmarked one, writes each tool skill's first body paragraph as its description, and refuses a selection whose written descriptions exceed the 900-token user listing budget |
...
| `basicly retention [response] [--baseline ...] [--verbose] [--strict]` | Score a recall answer against an always-on instruction file and report whether that file is in the session's context. It answers presence, not adherence: reciting a rule is not obeying it, and while the file is in context the answer is read from it rather than recalled. The `retention-probe` skill is the runbook |
...
| `basicly permissions-build` / `permissions-check` | Project the agent-permissions allow and deny lists into the co-owned settings file. It adds each missing managed pattern and removes nothing, so every entry the repository wrote itself stays, a blanket `Bash` allow included. The check reports a missing managed pattern. It also reads `~/.claude/settings.json` and warns when `autoMode.environment` has no `"$defaults"` and its trusted repo or source control entry names one repository, because auto mode then reads every other repository as external. The warning prints the fix. The check never writes that file |
...
| `basicly tracker import <export> [--source LABEL] [--dry-run] [--deleted ID]` | Import a foreign tracker's export (a beads `issues.jsonl`) into the owned ledger, preserving each source id so a commit message that references one still resolves. Unknown fields carry over verbatim and every event records its provenance. It is an upsert, never a sync: a record the ledger already holds is not re-created, a field disagreement is reported rather than reconciled, and absence from the export is never read as a deletion — pass `--deleted` for one you confirmed out of band. A re-run is a replay, so a torn import completes instead of doubling. `--dry-run` applies the same id rule, writes nothing, and exits with the code the real run would, so a scripted preflight refuses what the run refuses |
| `basicly tracker fold` | Fold every pending writer shard into the trunk log and commit the result, so a merged branch's events reach the log the readers read. The commit carries a record id taken from the shard's own events, so the commit-msg gate admits it. It refuses while the tree carries a change outside the ledger, and `basicly install` wires it to a `post-merge` hook so a consumer never has to remember it |
| `basicly tracker serve` | Serve the backlog as a web page and an HTTP API on `127.0.0.1:8765`, through the board kit. Each API endpoint runs the tracker kit command of the same name and returns its JSON. Writes remove machine paths the way the engine write seam does. `--web DIR` serves your own page |
| `basicly tracker scrub` | Rewrite the committed ledger without machine-specific paths or usernames, and print how many events changed. The named surface for the repair `tracker-path-scan` demands: an import necessarily carries the source's `created_by` and `source_repo_path`, and before this verb the only working repair was a `python -c` into a library function |
