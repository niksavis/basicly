...
This document does not tabulate the commands. The per-command reference is
[`docs/reference/cli.md`](../reference/cli.md), which carries a behaviour row for every
command and every command group, and the gates that keep it true (basicly-mfavrh moved it
there). A per-command table goes stale on every landing, and a specification must not go
stale on a schedule it does not control — [`conventions.md`](conventions.md) §8 makes that
the rule for the whole documentation set.

What is specified here is the shape of that surface, which is a decision and does not move
when a flag does. The commands fall into three surfaces: **lifecycle** for a consumer
repository, **catalog** for an author of catalog sources, and **harness** for the development
loop in either repository.
...
The 57 tiers hold 130 modules and group into 9 bands. Every band may import every band below
...
flowchart TB
  ...
  b2["2 · drivers — 11<br/>supervise · loop · release · usage_report"]
  ...
  b8["8 · tracker seam — 22<br/>owned_store · mirror · dispatch_phase · board_schema · board_fields"]
