...
| Tier injection, so a declared tier reaches the spawn | building | basicly-a3yi | The tier is declared on every agent source and validated by lint, and it reaches no spawn, because the hook that would rewrite one is not installed - this repository's `.claude/settings.json` carries `PreToolUse` entries for the edit tools and for `Bash`, and none for `Agent` [measured 2026-09-16]. **The earlier clause saying the claude installer declines with a nonzero exit is withdrawn as false** [measured 2026-09-16, `python3 .basicly/core/kit/tier/install_hook.py --dry-run` exits 0 and reports it would write the `PreToolUse`/`Agent` hook]. The hook returns `hookSpecificOutput.updatedInput`, which is a documented `PreToolUse` field, so the shape is right and only the firing is unproven. `CLAUDE_CODE_SUBAGENT_MODEL` outranks whatever the hook writes and `CLAUDE_CODE_SUBAGENT_MODEL_FORCE` outranks the spawn prompt and the definition too; both are unset here. On copilot the installer writes nothing and its stated reason is known-wrong - that host now ships hooks and carries per-subagent selection in config at `subagents.agents.<name>.{model, effortLevel, contextTier}`, which basicly-u2hl.41 wires |
...
| Acceptance criteria read from one store rather than two | designed | basicly-u07bfuw | Architecture D-50, migrated by corrective append under the rule section 32.8 binds - 150 open records need a field written, and the 1,020 closed ones keep the heading as evidence. `plan_record.section_entries` reads the heading today, so 467 records carry a field nothing tests against |
| The two acceptance stores reconciled where they disagree | designed | basicly-fjvdie6 | Architecture D-50. Measured 2026-09-13 - 202 records carry both stores and 94 disagree; on basicly-17nu the heading reads `todo` while the field holds real criteria. Read rather than scripted, because picking a store loses the other's content |
