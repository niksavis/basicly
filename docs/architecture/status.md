...
| Tier injection, so a declared tier reaches the spawn | building | basicly-a3yi | **The break is at projection, not at spawn** [measured 2026-09-16]. The catalog source declares the tier - `.basicly/core/agents/reviewer/agent.yaml` reads `tier: high` - and **0 of 11 projected files under `.claude/agents/` carry a `tier:` field**. `agents.py` parses the tier into the agent model and validates it against `MODEL_TIERS`, and nothing emits it into the rendered frontmatter, so the resolver reads a file the tier never reached. The hook is now installed here and correctly rewrites nothing. Two earlier clauses are withdrawn as false - the claude installer exits 0 and writes the `PreToolUse`/`Agent` entry, and the hook returns `hookSpecificOutput.updatedInput`, which is a documented field rather than a contract the host refuses. Whether the host calls the hook on a real spawn is still unproven; every probe fed the script a payload directly. On copilot the installer writes nothing and its stated reason is known-wrong - that host now ships hooks and carries per-subagent selection in config, which basicly-u2hl.41 wires |
