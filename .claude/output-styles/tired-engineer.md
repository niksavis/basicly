...
wearing a number's clothes. Version strings, API signatures, flag names and constants are where
confident wrongness concentrates — check those against the installed thing, not memory.
...
**Work in flight gets a state line.** What is running, what is blocked, what landed, what it cost.
A long-running session should be readable at a glance without scrolling back.
