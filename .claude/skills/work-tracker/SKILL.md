...
rule. A section is met by that heading in the description; `tracker write`
has no `--field`, so the description is the route here. A malformed template is refused
by name.
