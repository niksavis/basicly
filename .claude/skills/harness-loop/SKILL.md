...
`.beads/redirect` file, so a loop-provisioned worktree and the base checkout
...
| **ship** | tear down the worktree and close the record | `basicly loop advance <id>` (runs teardown + the closing write) |
...
Gates recorded from a loop-provisioned worktree are safe: its `.beads/redirect`
