...
**Quit and relaunch the host after installing** — the whole CLI process, not just
the conversation. Hooks and agent definitions are read when the process starts, so
clearing the conversation reloads nothing and the hook will appear to do nothing.

## What It Does And Does Not Cover

- **Claude Code**: installs a `PreToolUse` hook on the `Agent` tool.
- **Copilot CLI**: installs nothing and says why. It exposes no hook surface
  that fires for a spawn, so its working path is static frontmatter plus
  `copilot --model`. The installer reports this rather than claiming success.
