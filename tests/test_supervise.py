...
#
# The reported pass: three lanes, `VERDICT: ready`, and a `CHANGELOG.md` in nobody's
# scope that bounced the third lane twice. The lanes were hand-filed siblings, so
# `decompose` never grouped them — preflight is the only surface that sees the set,
# and the whole cost of missing it is a rework budget.
...
def test_the_report_warns_when_every_lane_appends_to_a_path_none_declares(
    ...
    lines = supervise.append_only_report(tmp_path, lanes, ("CHANGELOG.md",))
...
def test_the_report_leaves_out_a_lane_that_declared_the_path_itself(
    ...
    lines = supervise.append_only_report(tmp_path, ("a", "b", "c"), ("CHANGELOG.md",))
...
def test_the_report_says_when_nothing_is_declared_append_only(tmp_path: Path) -> None:
    ...
    """
    (line,) = supervise.append_only_report(tmp_path, ("a", "b"), ())
...
def test_a_single_lane_pass_contends_with_nobody(
    ...
    (line,) = supervise.append_only_report(tmp_path, ("only",), ("CHANGELOG.md",))
