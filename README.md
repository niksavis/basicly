...
Pin `@v0.18.3` for reproducible installs, or track `@main` for the latest. To
pin through the shim, append `-s -- --ref v0.18.3` (POSIX) or download the
script and pass `-Ref v0.18.3` (PowerShell). Where `git` is unavailable
...
```sh
uvx --from git+https://github.com/niksavis/basicly@v0.18.3 basicly uninstall
```
...
```sh
uvx --from git+https://github.com/niksavis/basicly@v0.18.3 basicly check   # exit non-zero when generated files drifted
uvx --from git+https://github.com/niksavis/basicly@v0.18.3 basicly build   # regenerate agent instruction files
```
