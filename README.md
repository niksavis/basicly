...
```sh
uvx --from git+https://github.com/niksavis/basicly@v0.18.9 basicly install
```
...
Pin `@v0.18.9` for reproducible installs, or track `@main` for the latest. To
pin through the shim, append `-s -- --ref v0.18.9` (POSIX) or download the
script and pass `-Ref v0.18.9` (PowerShell). Where `git` is unavailable
...
```sh
uvx --from git+https://github.com/niksavis/basicly@v0.18.9 basicly uninstall
```
...
```sh
uvx --from git+https://github.com/niksavis/basicly@v0.18.9 basicly check   # exit non-zero when generated files drifted
uvx --from git+https://github.com/niksavis/basicly@v0.18.9 basicly build   # regenerate agent instruction files
```
