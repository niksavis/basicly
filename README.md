...
```sh
uvx --from git+https://github.com/niksavis/basicly@v0.18.5 basicly install
```
...
Pin `@v0.18.5` for reproducible installs, or track `@main` for the latest. To
pin through the shim, append `-s -- --ref v0.18.5` (POSIX) or download the
script and pass `-Ref v0.18.5` (PowerShell). Where `git` is unavailable
...
```sh
uvx --from git+https://github.com/niksavis/basicly@v0.18.5 basicly uninstall
```
...
```sh
uvx --from git+https://github.com/niksavis/basicly@v0.18.5 basicly check   # exit non-zero when generated files drifted
uvx --from git+https://github.com/niksavis/basicly@v0.18.5 basicly build   # regenerate agent instruction files
```
