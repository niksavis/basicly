...
```sh
uvx --from git+https://github.com/niksavis/basicly@v0.6.0 basicly install
```
...
Pin `@v0.6.0` for reproducible installs, or track `@main` for the latest. To
pin through the shim, append `-s -- --ref v0.6.0` (POSIX) or download the
script and pass `-Ref v0.6.0` (PowerShell). Where `git` is unavailable
...
```sh
uvx --from git+https://github.com/niksavis/basicly@v0.6.0 basicly uninstall
```
...
```sh
uvx --from git+https://github.com/niksavis/basicly@v0.6.0 basicly check   # exit non-zero when generated files drifted
uvx --from git+https://github.com/niksavis/basicly@v0.6.0 basicly build   # regenerate agent instruction files
```
