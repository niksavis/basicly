...
`site/index.html` and both `.scripts/bootstrap.*` shims, assembles the
`changelog.d/` fragments into the dated `CHANGELOG.md` section, commits, and
...
The tag push triggers `.github/workflows/release.yml`, which extracts the tag's
`CHANGELOG.md` section, appends the pinned `uvx` install line, and creates the
GitHub release. Confirm the run went green and the release body matches that
section.

### 3. Replace the release page with the highlights

The workflow publishes the whole section, and a section assembled from a hundred
fragments is a wall nobody reads: v0.10.0 published 124,999 characters where
v0.9.0's page, rewritten by hand, is 3,050. The changelog is the record; the
release page is the summary of it. Until `basicly-xsdvp6` makes the workflow do
this, write the page yourself once the run is green, and keep the `## Install`
block the workflow appended:

```sh
gh release view vX.Y.Z --json body --jq .body > /tmp/page.md   # keep its ## Install block
gh release edit vX.Y.Z --notes-file /tmp/page.md               # after rewriting the rest
```

The shape, from v0.9.0 and v0.10.0: the heading, one paragraph saying what the
release is, one line with the entry counts per category and a link to the
section in `CHANGELOG.md` at the tag, a `### Breaking` list carrying every entry
whose bold lead says `BREAKING`, then three to five themed `###` sections of bold
one-line highlights. Under 6,000 characters. Nothing from `### Fixed` unless it
changes what a consumer must do.

This step is not in a gate, so it is the one most often forgotten: the v0.10.0
page went out as the wall first. Do it in the same session as the push.
