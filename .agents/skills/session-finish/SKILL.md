...
(git-ignored, survives between sessions). Read it and report:

```sh
cat .basicly/usage/tool-usage.json
```

- Entries keyed by plain name (`rg`, `uv`, `jq`, ...) are terminal tools;
  entries keyed `skill:<name>` are skill invocations.
- Report the top tools and skills by count with their `last_used` dates,
  and call out installed tools that never appear — they are candidates for
  culling from the harness.
- The file accumulates across sessions by design; do not reset or delete
  it. If it is missing, say so (the hook may not be active in this repo)
  and continue with the rest of the ritual.
