...
never edit one file and never conflict. A merge therefore lands a shard, and every read
folds the shards with the trunk log, so folding only bounds the file count:

```sh
basicly tracker fold
```

`basicly install` wires this to a `post-merge` hook only when `[tracker] fold_on_merge =
true`, and the hook folds only on the default branch. Leave it off where the default
branch takes pull requests: two different folds conflict on the forge
(basicly-k1pxru4.3). Run it by hand, as its own pull request, when `fsck` warns about the
shard count, or when the hook printed *the pending shards are not folded* because the
engine was not on the path. The commit it
...
`basicly policy dor <id>` owes a trigger, the acceptance criteria and the sections
`[policy.type_sections]` declares for the record's type. A `template.json` in the ledger
directory changes that for this engine and the standalone kit alike: `"mode": "extend"`
adds its `sections` and its per-type `types`, and `"mode": "override"` replaces the whole
rule. A template section is met by that heading with content in the description, the
same rule the kit applies; `tracker write` has no `--field`, so the description is the
route here. A malformed template is refused by name.
