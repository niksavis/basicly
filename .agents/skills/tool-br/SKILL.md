...
- Prefer `br update <id> --status in_progress` before starting work. For work done
  ...
  `chore: close <id>` commit with no other content. For loop-tracked work the close
  happens after the merge commit and cannot ride a work commit — follow the
  two-commits-one-push discipline in the `harness-loop` skill instead.
