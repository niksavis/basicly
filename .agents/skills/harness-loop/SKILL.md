...
The engine makes the only tracker commits, at the two natural points:

1. **At landing**, `loop advance` rolls all accumulated `.beads/**` dirt in
   base (claim, work type, checkpoints, gate records) into one
   `chore(beads)` commit before merging. A base that is dirty with anything
   *outside* `.beads/` still blocks — that is someone's uncommitted work.
2. **At ship**, after `br close`, the engine commits the closing tracker
   state itself.
...
it). **Push once per track**, after ship — CI ignores `.beads/**`-only
pushes by design (the local commit-msg hooks are the deterministic floor),
so batched pushes cost one CI run for the real change and none for the
tracker noise.
