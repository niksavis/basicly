...
Record gates from the base checkout only. A linked worktree carries a
throwaway tracker copy, so a gate recorded there is discarded at landing —
`basicly verify --issue` refuses to record from one for exactly this reason.
During build you never need to record the gate manually: the landing advance
verifies the rebased tree in the worktree and records the required gate from
base itself.
