...
```console
$ python3 .basicly/core/kit/install_hook.py --host copilot
...
```bash
python3 .basicly/core/kit/install_hook.py --dry-run   # print what it would write
python3 .basicly/core/kit/install_hook.py             # this repository
python3 .basicly/core/kit/install_hook.py --user      # every repository on this machine
```
...
```console
$ python3 .basicly/core/kit/tier_resolver.py --host claude --tier low
...
```console
$ python3 .basicly/core/kit/tier_resolver.py --host copilot --tier low
...
```console
$ python3 .basicly/core/kit/tier_resolver.py --host claude --name code-reviewer --default-tier medium
...
```console
$ python3 .basicly/core/kit/tier_resolver.py --host copilot --tier low --vendor google
...
```console
$ find . -type f
./kit/claude_tier_hook.py
./kit/install_hook.py
./kit/tier_resolver.py
./models/model-map.json

$ env -i python3 -S -I kit/tier_resolver.py --host claude --tier high --map models/model-map.json
...
$ printf '%s' '{"tool_name":"Agent","cwd":"'"$PWD"'","tool_input":{"subagent_type":"my-agent","prompt":"x"}}' \
    | python3 .basicly/core/kit/claude_tier_hook.py
...
- **No basicly, no third party, no network**, and it must stay parseable by an
  interpreter older than this repo's 3.14 floor: no syntax newer than 3.9, and one
  exception class per handler. This repo's `ruff format` targets 3.14 and will
  rewrite a parenthesized multi-exception `except` into syntax a consumer's Python
  may not have.
...
The design note is [`docs/design/tier-injection-kit.md`](../../../docs/design/tier-injection-kit.md).
