...
def _run_br(
    ...
    """Run a ``br`` subcommand. Raises if ``br`` is absent — the decomposer needs the tracker."""
    br = shutil.which("br")
    if not br:
        raise RuntimeError("br is not on PATH; the decomposer requires the beads tracker")
...
def decompose(repo_root: Path, feature_id: str, children: tuple[ChildSpec, ...]) -> DecomposeResult:
    ...
    Each child is created with acceptance criteria (so DoR passes), then any two
    children whose declared scopes overlap are serialized by a ``blocks`` chain
    in declared order. The resulting graph is checked for cycles before the
    result — carrying the parallel groups and serial order — is returned.
    """
    if not children:
